...
int caller(int num_images, int num_threads)
{
    ...
    double init_time = end-beg;
	cout<<"Time to initialize filters: "<<init_time<<endl;
    ...
    double load_time = end-beg;
	cout<<"Time to load dataset: "<<load_time<<endl<<"Loaded file: "<<filename<<endl;
	...
	#pragma omp parallel
	{
        ...
        init = omp_get_wtime() - init;
        printf( "Time taken by thread %d is %f\n", omp_get_thread_num(), init );
    }
    ...
    if(num_threads==1){
        cout<<"Code is serial!"<<endl;
        serial_time=par_time; // global variable assigned
    }
    ...
    fstream datalog;
    datalog.open("data.csv",ios::app);
    stringstream ss;
    ss << num_threads << "," << total_time << "," << frac_enh << "," << speedup << ",\n";
    datalog << ss.str();
    datalog.close();
...
int main(int argc, char const *argv[]){
    ...
    for (int num : all_num_threads) {
        caller(num_images, num);
    }
    return 0;
}